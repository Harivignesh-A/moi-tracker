  ...
  try {
    const response =
      await gapi.client.sheets.spreadsheets.values.batchGet({
        ...
        valueRenderOption: "FORMATTED_VALUE", // Changed to preserve exact text strings like "3 gm gold"
      });
...
function parseAmount(value) {
  ...
  // Clean potential currency symbols or commas to check if it's purely a number
  const cleanedNumStr = strVal.replace(/[₹,\s]/g, "");

  // If it translates to a valid number, return as numeric value
  if (cleanedNumStr !== "" && !isNaN(Number(cleanedNumStr))) {
    return Number(cleanedNumStr);
  }

  // Otherwise, return null to indicate this is a descriptive text gift (e.g., "3 gm gold")
  return null;
}
...
function showPerson(name) {
  ...
  // Extract non-numeric text gifts
  ...
  // Display text gift statuses if present
  if (textGiftsReceived.length > 0) {
    statusMessages.push(`${name} has gifted ${textGiftsReceived.join(", ")}`);
  }
  if (textGiftsGiven.length > 0) {
    statusMessages.push(`You have gifted ${textGiftsGiven.join(", ")}`);
  }

  // Handle cash balance messages
  if (balance > 0) {
    statusMessages.push(`${name} should give you ${formatCurrency(balance)}`);
  } else if (balance < 0) {
    statusMessages.push(`You have to pay ${name} ${formatCurrency(balance)}`);
  ...
  // Display final combined message
  ...
  if (!transactions.length) {
    ...
    for (const row of transactions) {
      ...
      // Display text string directly in transaction table
