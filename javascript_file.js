...
function showPerson(name) {
  const given = givenRows
    ...
    .map(row => {
      const rawVal = row[4];
      const parsedAmount = toAmount(rawVal);
      const isText = parsedAmount === 0 && String(rawVal ?? "").trim() !== "";
      return {
        type: "Given",
        date: row[2] ?? "",
        function: row[3] ?? "",
  ...
  const nonNumericGiven = given.filter(r => r.isTextAmount).map(r => r.rawAmount);
  const nonNumericReceived = received.filter(r => r.isTextAmount).map(r => r.rawAmount);

  $("personName").textContent = name;
  $("givenTotal").textContent =
  ...
  if (balance > 0) {
    statusText = `${name} should give you ${formatCurrency(balance)}`;
  } else if (balance < 0) {
    statusText = `You have to pay ${name} ${formatCurrency(balance)}`;
    message.classList.add("owe");
  } else {
    statusText = "Settled — no amount pending";
    message.classList.add("settled");
  }

  let textGiftNotes = [];
  if (nonNumericGiven.length > 0) {
    textGiftNotes.push(`You gave: ${nonNumericGiven.join(", ")}`);
  }
  if (nonNumericReceived.length > 0) {
    textGiftNotes.push(`Received: ${nonNumericReceived.join(", ")}`);
  }

  if (textGiftNotes.length > 0) {
    if (balance === 0) {
      message.textContent = textGiftNotes.join(" | ");
      message.classList.remove("settled");
    } else {
      message.textContent = `${statusText} (${textGiftNotes.join(" | ")})`;
    }
  } else {
  ...
  if (!transactions.length) {
    ...
    for (const row of transactions) {
      ...
      const displayAmount = row.isTextAmount
